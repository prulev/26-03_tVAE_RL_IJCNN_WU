...
s = subplot(221);
plot_latent_space_by_trial_traj(test_data, [1 3])
...
% text(0.95, 0.05, "\textbf{latent~dimension~1,~3}", 'Interpreter','latex', ...
%   'FontSize',14, 'Units','normalized', 'HorizontalAlignment','right')
text(-0.1, 1.1, "\textbf{(a)}", 'Interpreter','latex', ...
  'FontSize',14, 'Units','normalized', 'HorizontalAlignment','right')
...
leg = legend([temp(5) temp(103) temp(248) temp(6)], ...
  ...
  'Position',[0.261833295165926 0.955083333333333 0.651733409668148 0.0279]);
...
% text(0.95, 0.05, "\textbf{latent~dimension~1,~4}", 'Interpreter','latex', ...
%   'FontSize',14, 'Units','normalized', 'HorizontalAlignment','right')
...
subplot(223)
plot_latent_space_by_trial_traj(test_data, [1 4])
title("\textbf{Rat~B~mPFC~latent~dynamics}", 'Interpreter','latex', 'FontSize',15, 'FontWeight','bold')
% text(0.95, 0.95, "\textbf{latent~dimension~1,~4}", 'Interpreter','latex', ...
%   'FontSize',14, 'Units','normalized', 'HorizontalAlignment','right')
xlabel("\bf{latent dimension 1}", 'Interpreter','latex', 'FontSize',14)
ylabel("\bf{latent dimension 4}", 'Interpreter','latex', 'FontSize',14)
text(-0.1, 1.1, "\textbf{(c)}", 'Interpreter','latex', ...
  'FontSize',14, 'Units','normalized', 'HorizontalAlignment','right')
...
test_data = load(['latent_models_backup/tVAE_' rat '_' region '_' num2str(test_fold-1) '_1En1De_' num2str(latent_dim) 'latent_decoder_pos.mat' ], ...
    'latent_mu', 'latent_std', 'movements', 'trials');
subplot(224)
plot_latent_space_by_trial_traj(test_data, [2 4])
title("\textbf{Rat~B~M1~latent~dynamics}", 'Interpreter','latex', 'FontSize',15, 'FontWeight','bold')
% text(0.95, 0.95, "\textbf{latent~dimension~2,~4}", 'Interpreter','latex', ...
%   'FontSize',14, 'Units','normalized', 'HorizontalAlignment','right')
xlabel("\bf{latent dimension 2}", 'Interpreter','latex', 'FontSize',14)
ylabel("\bf{latent dimension 4}", 'Interpreter','latex', 'FontSize',14)
text(-0.1, 1.1, "\textbf{(d)}", 'Interpreter','latex', ...
  'FontSize',14, 'Units','normalized', 'HorizontalAlignment','right')
...
for trialIdx = 1:size(low_traj, 1)
  ...
  hold on
  plot(squeeze(low_traj(trialIdx,201:251,plot_dim(1))), squeeze(low_traj(trialIdx,201:251,plot_dim(2))), 'b', 'LineWidth',1)
  ...
  plot(squeeze(low_traj(trialIdx,552:651,plot_dim(1))), squeeze(low_traj(trialIdx,552:651,plot_dim(2))), 'g', 'LineWidth',1)
...
for trialIdx = 1:size(high_traj, 1)
  ...
  hold on
  plot(squeeze(high_traj(trialIdx,201:251,plot_dim(1))), squeeze(high_traj(trialIdx,201:251,plot_dim(2))), 'b', 'LineWidth',1)
  ...
  plot(squeeze(high_traj(trialIdx,552:651,plot_dim(1))), squeeze(high_traj(trialIdx,552:651,plot_dim(2))), 'r', 'LineWidth',1)
