function plotActionsWithPos(index, actions, color, xRange, pos)
temp = selectOneAction(actions, 1);
plot(index, pos*temp, 'Color', color(1,:), 'LineWidth', 12);
hold on;
temp = selectOneAction(actions, 2);
plot(index, pos*temp, 'Color', color(2,:), 'LineWidth', 12);
temp = selectOneAction(actions, 3);
plot(index, pos*temp, 'Color', color(3,:), 'LineWidth', 12);
hold off
ylim([0.8 1.2])
xlim(xRange)
box off
