...
def run_exp(config, dataset, paths):
    ...
    # this use TC_tVAE, Ziyi 260407
    experiment = f'TCtVAE_{config.DATA.RAT}_{config.DATA.REGION}_{config.DATA.TEST_FOLD}_' \
                 f'{config.MODEL.N_LAYERS_ENCODER}En' \
                 f'{config.MODEL.N_LAYERS_DECODER}De_' \
                 f'{config.MODEL.LATENT_DIM}latent_' \
                 f'{config.TRAIN.GAMMA}gamma_' \
                 f'{paths}'
    print(f'Experiment {experiment} start.')
    runner = Runner(config, dataset)
...
def main():
    ...
    '''
    acquire TC-tVAE encoder and decoder for both rat028 mPFC and M1 with_decoder_pos
    '''
    for region in ['mPFC', 'M1']:  # ['mPFC', 'M1']:
        for test_fold in [4]:
            dataset = Dataset(default_config, rat, region, test_fold, device)
            for latent_dim in [4]:  # [6]:
                ...
                if rat == '025':  # the number of  025's data samples allows more parameters
                    embed_dim = 24
                config = get_config('TCtVAE_Variational_With_decoder_pos', [
                    'DATA.RAT', rat,
                    'DATA.REGION', region,
                    'DATA.TEST_FOLD', test_fold,
                    'MODEL.LATENT_DIM', latent_dim,
